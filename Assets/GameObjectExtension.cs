...
public static class GameObjectExtension
{
    ...
    public static Entity FindEntityInChildWithouthTag<Entity>(this GameObject parent, string tag) where Entity : class
    {
        Entity[] entities = parent.GetComponentsInChildren<Entity>();

        return FindItFromRefusedTag<Entity>(entities, tag);
    }
    ...
    public static Entity FindItFromRefusedTag<Entity>(Entity[] entities, string tagToExclude)
    {
        foreach (Entity entity in entities)
        {
            if ((entity as ICanvasElement).transform.tag != tagToExclude)
            {
                return entity;
            }
        }
